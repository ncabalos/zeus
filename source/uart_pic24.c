...
void UartHardwareInit(uint8_t uart_num, uint32_t baud_rate, uint16_t stop_bits,
                      uint16_t parity)
{
    switch (uart_num) {
        case 0:
            Uart1Init(baud_rate, stop_bits, parity);
            break;

        case 1:
            Uart2Init(baud_rate, stop_bits, parity);
            break;

        case 2:
            Uart3Init(baud_rate, stop_bits, parity);
            break;

        case 3:
            Uart4Init(baud_rate, stop_bits, parity);
            break;
    }
}
...
void Uart1Init(uint32_t baud_rate, uint16_t stop_bits, uint16_t parity)
{
    ...
    CloseUART1();
    ConfigIntUART1(UART_RX_INT_EN | UART_RX_INT_PR6 | UART_TX_INT_EN |
                   UART_TX_INT_PR6);
    OpenUART1(UART_EN | parity_config | stopbit_config | brg_config, UART_TX_ENABLE,
              brg_value);
}
...
void Uart2Init(uint32_t baud_rate, uint16_t stop_bits, uint16_t parity)
{
    ...
    brg_value = get_brg(baud_rate, brg_config);
    CloseUART2();
    ConfigIntUART2(UART_RX_INT_EN | UART_RX_INT_PR6 | UART_TX_INT_EN |
                   UART_TX_INT_PR6);
    OpenUART2(UART_EN | parity_config | stopbit_config | brg_config, UART_TX_ENABLE,
              brg_value);
}
...
void Uart3Init(uint32_t baud_rate, uint16_t stop_bits, uint16_t parity)
{
    ...
    brg_value = get_brg(baud_rate, brg_config);
    CloseUART3();
    ConfigIntUART3(UART_RX_INT_EN | UART_RX_INT_PR6 | UART_TX_INT_EN |
                   UART_TX_INT_PR6);
    OpenUART3(UART_EN | parity_config | stopbit_config | brg_config, UART_TX_ENABLE,
              brg_value);
}
...
void Uart4Init(uint32_t baud_rate, uint16_t stop_bits, uint16_t parity)
{
    ...
    brg_value = get_brg(baud_rate, brg_config);
    CloseUART4();
    ConfigIntUART4(UART_RX_INT_EN | UART_RX_INT_PR6 | UART_TX_INT_EN |
                   UART_TX_INT_PR6);
    OpenUART4(UART_EN | parity_config | stopbit_config | brg_config, UART_TX_ENABLE,
              brg_value);
}
...
uint16_t get_parity(uint16_t parity)
{
    switch (parity) {
        case SERIO_NOPARITY:
            return UART_NO_PAR_8BIT;

        case SERIO_ODD:
            return UART_ODD_PAR_8BIT;

        case SERIO_EVEN:
            return UART_EVEN_PAR_8BIT;
    }
...
uint16_t get_stopbit(uint16_t stop_bits)
{
    switch (stop_bits) {
        case SERIO_1STOPBIT:
            return UART_1STOPBIT;

        case SERIO_2STOPBIT:
            return UART_2STOPBITS;
    }
