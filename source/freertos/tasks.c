...
typedef struct tskTaskControlBlock {
    ...
    signed char
    pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */
...
PRIVILEGED_DATA static xList * volatile
...
PRIVILEGED_DATA static xTaskHandle xIdleTaskHandle =
...
PRIVILEGED_DATA static volatile unsigned portBASE_TYPE uxCurrentNumberOfTasks
    = ( unsigned portBASE_TYPE ) 0U;
PRIVILEGED_DATA static volatile portTickType xTickCount 						=
    ( portTickType ) 0U;
...
PRIVILEGED_DATA static volatile unsigned portBASE_TYPE uxPendedTicks 			=
    ( unsigned portBASE_TYPE ) 0U;
...
PRIVILEGED_DATA static unsigned portBASE_TYPE uxTaskNumber 						=
    ( unsigned portBASE_TYPE ) 0U;
PRIVILEGED_DATA static volatile portTickType xNextTaskUnblockTime				=
    portMAX_DELAY;
...
PRIVILEGED_DATA static unsigned long ulTotalRunTime =
...
extern void vApplicationStackOverflowHook( xTaskHandle xTask,
...
static unsigned portBASE_TYPE prvListTaskWithinSingleList(
    xTaskStatusType * pxTaskStatusArray, xList * pxList,
...
static unsigned short prvTaskCheckFreeStackSpace( const unsigned char *
        ...
        const xMemoryRegion * const xRegions )
{
    ...
    configASSERT( ( ( uxPriority & ( ~portPRIVILEGE_BIT ) ) <
    ...
    if( pxNewTCB != NULL ) {
...
#else /* portSTACK_GROWTH */
        {
            ...
            configASSERT( ( ( ( unsigned long ) pxNewTCB->pxStack &
...
void vTaskDelete( xTaskHandle xTaskToDelete )
{
    ...
    taskENTER_CRITICAL();
    {
        ...
        if( uxListRemove( &( pxTCB->xGenericListItem ) ) == ( unsigned portBASE_TYPE )
            0 ) {
...
eTaskState eTaskGetState( xTaskHandle xTask )
{
    ...
    if( pxTCB == pxCurrentTCB ) {
        ...
        taskENTER_CRITICAL();
        {
            pxStateList = ( xList *) listLIST_ITEM_CONTAINER( &
...
void vTaskPrioritySet( xTaskHandle xTask, unsigned portBASE_TYPE uxNewPriority )
{
    ...
    if( uxNewPriority >= ( unsigned portBASE_TYPE ) configMAX_PRIORITIES ) {
        uxNewPriority = ( unsigned portBASE_TYPE ) configMAX_PRIORITIES -
    ...
    taskENTER_CRITICAL();
    {
        ...
        if( uxCurrentBasePriority != uxNewPriority ) {
            ...
            listSET_LIST_ITEM_VALUE( &( pxTCB->xEventListItem ),
                                     ( ( portTickType ) configMAX_PRIORITIES - ( portTickType )
...
void vTaskStartScheduler( void )
{
...
#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
    {
        ...
        xReturn = xTaskCreate( prvIdleTask, ( signed char *) "IDLE", tskIDLE_STACK_SIZE,
        ...
        unsigned long * pulTotalRunTime )
{
    ...
    vTaskSuspendAll();
    {
        ...
        if( uxArraySize >= uxCurrentNumberOfTasks ) {
            ...
            } while( uxQueue > ( unsigned portBASE_TYPE )
                     tskIDLE_PRIORITY ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */

            /* Fill in an xTaskStatusType structure with information on each
            task in the Blocked state. */
            uxTask += prvListTaskWithinSingleList( &( pxTaskStatusArray[ uxTask ] ),
                                                   ( xList *) pxDelayedTaskList, eBlocked );
            uxTask += prvListTaskWithinSingleList( &( pxTaskStatusArray[ uxTask ] ),
                                                   ( xList *) pxOverflowDelayedTaskList, eBlocked );
#if( INCLUDE_vTaskDelete == 1 )
            {
    ...
    if( uxSchedulerSuspended == ( unsigned portBASE_TYPE ) pdFALSE ) {
...
#if ( ( configUSE_PREEMPTION == 1 ) && ( configUSE_TIME_SLICING == 1 ) )
        {
            if( listCURRENT_LIST_LENGTH( &
                                         ( pxReadyTasksLists[ pxCurrentTCB->uxPriority ] ) ) > ( unsigned portBASE_TYPE )
                1 ) {
                xSwitchRequired = pdTRUE;
            }
        }
                                       ...
                                       const xMemoryRegion * const xRegions, unsigned short usStackDepth )
{
    ...
    for( x = ( unsigned portBASE_TYPE ) 0;
         x < ( unsigned portBASE_TYPE ) configMAX_TASK_NAME_LEN; x++ ) {
    ...
    if( uxPriority >= ( unsigned portBASE_TYPE ) configMAX_PRIORITIES ) {
        uxPriority = ( unsigned portBASE_TYPE ) configMAX_PRIORITIES -
                     ( unsigned portBASE_TYPE ) 1U;
    }
...
static void prvInitialiseTaskLists( void )
{
    ...
    for( uxPriority = ( unsigned portBASE_TYPE ) 0U;
         uxPriority < ( unsigned portBASE_TYPE ) configMAX_PRIORITIES; uxPriority++ ) {
                                        ...
                                        portSTACK_TYPE * puxStackBuffer )
{
    ...
    if( pxNewTCB != NULL ) {
        ...
        pxNewTCB->pxStack = ( portSTACK_TYPE *) pvPortMallocAligned( ( ( (
                                size_t ) usStackDepth ) * sizeof( portSTACK_TYPE ) ),
...
static unsigned portBASE_TYPE prvListTaskWithinSingleList(
    xTaskStatusType * pxTaskStatusArray, xList * pxList, eTaskState eState )
{
    ...
    if( listCURRENT_LIST_LENGTH( pxList ) > ( unsigned portBASE_TYPE ) 0 ) {
        ...
        do {
            ...
            pxTaskStatusArray[ uxTask ].pcTaskName = ( const signed char *) &
...
#if ( portSTACK_GROWTH > 0 )
            {
                ppxTaskStatusArray[ uxTask ].usStackHighWaterMark =
                    prvTaskCheckFreeStackSpace( ( unsigned char *) pxNextTCB->pxEndOfStack );
            }
#else
            {
                pxTaskStatusArray[ uxTask ].usStackHighWaterMark = prvTaskCheckFreeStackSpace( (
                            unsigned char *) pxNextTCB->pxStack );
            }
#endif
            uxTask++;
...
#endif
    uxReturn = ( unsigned portBASE_TYPE ) prvTaskCheckFreeStackSpace(
                   pcEndOfStack );
    return uxReturn;
}
