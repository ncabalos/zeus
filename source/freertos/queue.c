...
typedef struct QueueDefinition {
    ...
    union {								/* Use of a union is an exception to the coding standard to ensure two mutually exclusive structure members don't appear simultaneously (wasting RAM). */
        ...
        uxRecursiveCallCount;/*< Maintains a count of the numebr of times a recursive mutex has been recursively 'taken' when the structure is used as a mutex. */
    } u;
...
static void prvCopyDataFromQueue( xQUEUE * const pxQueue,
                                  const void * const pvBuffer ) PRIVILEGED_FUNCTION;
 ...
 */
static portBASE_TYPE prvNotifyQueueSetContainer( const xQUEUE * const pxQueue,
...
portBASE_TYPE xQueueGenericReset( xQueueHandle xQueue, portBASE_TYPE xNewQueue )
{
    ...
    taskENTER_CRITICAL();
    {
        ...
        pxQueue->u.pcReadFrom = pxQueue->pcHead + ( ( pxQueue->uxLength -
                                ( unsigned portBASE_TYPE ) 1U ) * pxQueue->uxItemSize );
...
xQueueHandle xQueueCreateMutex( unsigned char ucQueueType )
{
    ...
    if( pxNewQueue != NULL ) {
        ...
        ( void ) xQueueGenericSend( pxNewQueue, NULL, ( portTickType ) 0U,
                                    queueSEND_TO_BACK );
    } else {
...
portBASE_TYPE xQueueGiveMutexRecursive( xQueueHandle xMutex )
{
    ...
    if( pxMutex->pxMutexHolder == ( void *)
        xTaskGetCurrentTaskHandle() ) { /*lint !e961 Not a redundant cast as xTaskHandle is a typedef. */
                                        ...
                                        portTickType xBlockTime )
{
    ...
    if( pxMutex->pxMutexHolder == ( void *)
        xTaskGetCurrentTaskHandle() ) { /*lint !e961 Cast is not redundant as xTaskHandle is a typedef. */
        ( pxMutex->u.uxRecursiveCallCount )++;
        xReturn = pdPASS;
    } else {
...
{
    xQueueHandle xHandle;
    xHandle = xQueueGenericCreate( uxCountValue, queueSEMAPHORE_QUEUE_ITEM_LENGTH,
                                        ...
                                        portBASE_TYPE xCopyPosition )
{
    ...
    configASSERT( !( ( pvItemToQueue == NULL ) &&
                     ( pxQueue->uxItemSize != ( unsigned portBASE_TYPE ) 0U ) ) );
...
signed portBASE_TYPE xQueueAltGenericReceive( xQueueHandle xQueue,
        void * const pvBuffer, portTickType xTicksToWait, portBASE_TYPE xJustPeeking )
{
    signed portBASE_TYPE xEntryTimeSet = pdFALSE;
    xTimeOutType xTimeOut;
                ...
                if( xJustPeeking == pdFALSE ) {
...
#if ( configUSE_MUTEXES == 1 )
                    {
                        if( pxQueue->uxQueueType == queueQUEUE_IS_MUTEX ) {
                            /* Record the information required to implement
                            priority inheritance should it become necessary. */
                            pxQueue->pxMutexHolder = ( signed char *)
                                                     xTaskGetCurrentTaskHandle();  /*lint !e961 Cast is not redundant as xTaskHandle is a typedef. */
                        }
                    }
#endif
...
signed portBASE_TYPE xQueueReceiveFromISR( xQueueHandle xQueue,
        const void * const pvBuffer, signed portBASE_TYPE * pxHigherPriorityTaskWoken )
{
    signed portBASE_TYPE xReturn;
    unsigned portBASE_TYPE uxSavedInterruptStatus;
    xQUEUE * const pxQueue = ( xQUEUE *) xQueue;
...
signed portBASE_TYPE xQueueIsQueueEmptyFromISR( const xQueueHandle xQueue )
{
    ...
    if( ( ( xQUEUE *) xQueue )->uxMessagesWaiting == ( unsigned portBASE_TYPE )
        0 ) {
        xReturn = pdTRUE;
    } else {
        xReturn = pdFALSE;
...
signed portBASE_TYPE xQueueIsQueueFullFromISR( const xQueueHandle xQueue )
{
    ...
    if( ( ( xQUEUE *) xQueue )->uxMessagesWaiting == ( ( xQUEUE *)
            xQueue )->uxLength ) {
        xReturn = pdTRUE;
    } else {
        xReturn = pdFALSE;
...
signed portBASE_TYPE xQueueCRReceive( xQueueHandle xQueue, void * pvBuffer,
...
signed portBASE_TYPE xQueueCRSendFromISR( xQueueHandle xQueue,
        const void * pvItemToQueue, signed portBASE_TYPE xCoRoutinePreviouslyWoken )
{
    xQUEUE * const pxQueue = ( xQUEUE *) xQueue;
...
signed portBASE_TYPE xQueueCRReceiveFromISR( xQueueHandle xQueue,
        void * pvBuffer, signed portBASE_TYPE * pxCoRoutineWoken )
{
    signed portBASE_TYPE xReturn;
    xQUEUE * const pxQueue = ( xQUEUE *) xQueue;
...
void vQueueAddToRegistry( xQueueHandle xQueue, signed char * pcQueueName )
{
    ...
    a free slot. */
    for( ux = ( unsigned portBASE_TYPE ) 0U;
         ux < ( unsigned portBASE_TYPE ) configQUEUE_REGISTRY_SIZE; ux++ ) {
...
xQueueSetHandle xQueueCreateSet( unsigned portBASE_TYPE uxEventQueueLength )
{
    ...
    pxQueue = xQueueGenericCreate( uxEventQueueLength, sizeof( xQUEUE *),
                                   queueQUEUE_TYPE_SET );
    return pxQueue;
}
                              ...
                              xQueueSetHandle xQueueSet )
{
    ...
    if( ( ( xQUEUE *) xQueueOrSemaphore )->pxQueueSetContainer != NULL ) {
        ...
        xReturn = pdFAIL;
    } else if( ( ( xQUEUE *) xQueueOrSemaphore )->uxMessagesWaiting !=
                                   ...
                                   xQueueSetHandle xQueueSet )
{
    ...
    if( pxQueueOrSemaphore->pxQueueSetContainer != xQueueSet ) {
        ...
        xReturn = pdFAIL;
    } else if( pxQueueOrSemaphore->uxMessagesWaiting != ( unsigned portBASE_TYPE )
               0 ) {
        ...
        portTickType xBlockTimeTicks )
{
    ...
    ( void ) xQueueGenericReceive( ( xQueueHandle ) xQueueSet, &xReturn,
                                   xBlockTimeTicks,
...
{
    xQueueSetMemberHandle xReturn = NULL;
    ( void ) xQueueReceiveFromISR( ( xQueueHandle ) xQueueSet, &xReturn,
