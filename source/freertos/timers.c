...
static portBASE_TYPE prvInsertTimerInActiveList( xTIMER * pxTimer,
        portTickType xNextExpiryTime, portTickType xTimeNow,
 ...
 */
static void prvProcessExpiredTimer( portTickType xNextExpireTime,
                                    portTickType xTimeNow ) PRIVILEGED_FUNCTION;
...
static void prvProcessTimerOrBlockTask( portTickType xNextExpireTime,
                                        portBASE_TYPE xListWasEmpty ) PRIVILEGED_FUNCTION;
                                    ...
                                    portTickType xTimeNow )
{
    ...
    if( pxTimer->uxAutoReload == ( unsigned portBASE_TYPE ) pdTRUE ) {
        ...
        if( prvInsertTimerInActiveList( pxTimer,
            ...
            list.  Reload it now.  */
            xResult = xTimerGenericCommand( pxTimer, tmrCOMMAND_START, xNextExpireTime,
                                            NULL, tmrNO_DELAY );
...
{
    portTickType xTimeNow;
    PRIVILEGED_DATA static portTickType xLastTime = ( portTickType )
...
static portBASE_TYPE prvInsertTimerInActiveList( xTIMER * pxTimer,
        portTickType xNextExpiryTime, portTickType xTimeNow, portTickType xCommandTime )
{
    ...
    while( xQueueReceive( xTimerQueue, &xMessage,
        ...
        switch( xMessage.xMessageID ) {
            case tmrCOMMAND_START :
                ...
                if( prvInsertTimerInActiveList( pxTimer,
                                                xMessage.xMessageValue + pxTimer->xTimerPeriodInTicks, xTimeNow,
...
static void prvCheckForValidListAndQueue( void )
{
    ...
    taskENTER_CRITICAL();
    {
        if( xTimerQueue == NULL ) {
            ...
            xTimerQueue = xQueueCreate( ( unsigned portBASE_TYPE ) configTIMER_QUEUE_LENGTH,
                                        sizeof( xTIMER_MESSAGE ) );
        }
    }
    taskEXIT_CRITICAL();
}
...
portBASE_TYPE xTimerIsTimerActive( xTimerHandle xTimer )
{
    ...
    taskENTER_CRITICAL();
    {
        ...
        xTimerIsInActiveList = !( listIS_CONTAINED_WITHIN( NULL,
